...
/**
 * Changes the remark of an existing supplier in the address book.
 */
...
``` java
    public static final String MESSAGE_USAGE = COMMAND_WORD + ": Edits the remark of the supplier identified "
            + "by the index number used in the last supplier listing. "
...
public class RemarkCommand extends Command {
    ...
    /**
     * @param index of the supplier in the filtered supplier list to edit the remark
     * @param remark of the supplier to be updated to
     */
...
Create a `RemarkCommandParser` class in the `seedu.clinic.logic.parser` package. The class must extend the `Parser` interface.
...
Create a new `Remark` in `seedu.clinic.model.supplier`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
Without getting too deep into `fxml`, let’s go on a 5 minute adventure to get some placeholder text to show up for each supplier.

Simply add the following to [`seedu.clinic.ui.SupplierCard`](https://github.com/se-edu/addressbook-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-0c6b6abcfac8c205e075294f25e851fe).
...
Then insert the following into [`main/resources/view/SupplierListCard.fxml`](https://github.com/se-edu/addressbook-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-12580431f55d7880578aa4c16f249e71).

**`SupplierListCard.fxml`:**
...
## Modify `Supplier` to support a `Remark` field

Since `SupplierCard` displays data from a `Supplier`, we need to update `Supplier` to get our `Remark` displayed!

### Modify `Supplier`

We change the constructor of `Supplier` to take a `Remark`. We will also need to define new fields and accessors accordingly to store our new addition.

### Update other usages of `supplier`

Unfortunately, a change to `supplier` will cause other commands to break, you will have to modify these commands to use the updated `supplier`!

<div markdown="span" class="alert alert-primary">

:bulb: Use the `Find Usages` feature in IntelliJ IDEA on the `supplier` class to find these commands.
...
AddressBook stores data by serializing `JsonAdaptedSupplier` into `json` with the help of an external library — Jackson. Let’s update `JsonAdaptedSupplier` to work with our new `supplier`!
...
Now that we have finalized the `Supplier` class and its dependencies, we can now bind the `Remark` field to the UI.
...
In this last step, we modify `RemarkCommand#execute()` to change the `Remark` of a `Supplier`. Since all fields in a `Supplier` are immutable, we create a new instance of a `Supplier` with the values that we want and
save it with `Model#setSupplier()`.

**`RemarkCommand.java`:**

``` java
//...
    public static final String MESSAGE_ADD_REMARK_SUCCESS = "Added remark to Supplier: %1$s";
    public static final String MESSAGE_DELETE_REMARK_SUCCESS = "Removed remark from Supplier: %1$s";
//...
    @Override
    public CommandResult execute(Model model) throws CommandException {
        ...
        Supplier editedSupplier = new Supplier(supplierToEdit.getName(), supplierToEdit.getPhone(), supplierToEdit.getEmail(),
                supplierToEdit.getAddress(), remark, supplierToEdit.getTags());

        model.setSupplier(supplierToEdit, editedSupplier);
        model.updateFilteredSupplierList(PREDICATE_SHOW_ALL_SUPPLIERS);

        return new CommandResult(generateSuccessMessage(editedSupplier));
    }
     ...
     * {@code supplierToEdit}.
     */
    private String generateSuccessMessage(Supplier supplierToEdit) {
        String message = !remark.value.isEmpty() ? MESSAGE_ADD_REMARK_SUCCESS : MESSAGE_DELETE_REMARK_SUCCESS;
        return String.format(message, supplierToEdit);
    }
```
