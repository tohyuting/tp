package seedu.clinic.testutil;

import seedu.clinic.commons.core.index.Index;

/**
 * A utility class containing a list of {@code Index} objects to be used in tests.
 */
public class TypicalIndexes {
    public static final Index INDEX_FIRST_SUPPLIER = Index.fromOneBased(1);
    public static final Index INDEX_SECOND_SUPPLIER = Index.fromOneBased(2);
    public static final Index INDEX_THIRD_SUPPLIER = Index.fromOneBased(3);

    public static final Index INDEX_FIRST_WAREHOUSE = Index.fromOneBased(1);
    public static final Index INDEX_SECOND_WAREHOUSE = Index.fromOneBased(2);
    public static final Index INDEX_THIRD_WAREHOUSE = Index.fromOneBased(3);
}
